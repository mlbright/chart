...
update_worker_version:
	@echo ${ENGINE_VERSION}
	ytool -f "./codacy/values.yaml" -s worker-manager.config.codacy.worker.imageVersion "$(shell grep "engine" -A 2 codacy/requirements.lock | grep version | cut -d : -f 2 | tr -d '[:blank:]')" -e
